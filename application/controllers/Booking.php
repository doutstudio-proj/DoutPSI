...
class Booking extends EA_Controller
{
    ...
    public function register(): void
    {
        try {
            ...
            $cpf = $customer['custom_field_1'] ?? '';
            
            // Remove qualquer caractere não numérico
            $cpf = preg_replace('/[^0-9]/', '', $cpf);
            $customer['custom_field_1'] = $cpf;
            
            if (empty($cpf)) {
                throw new InvalidArgumentException('O CPF é obrigatório.');
            }

            $this->load->model('customers_model');
            $existing_customers = $this->customers_model->get(['custom_field_1' => $cpf]);
            
            if (empty($existing_customers)) {
                throw new RuntimeException('Apenas pacientes já cadastrados na clínica podem realizar agendamentos pelo portal. CPF não encontrado.');
            }

            // Vincula ao paciente existente para não criar duplicado
            $customer['id'] = $existing_customers[0]['id'];
            
            // Se o e-mail estiver em branco, usamos o e-mail existente para não quebrar a validação
            if (empty($customer['email'])) {
                $customer['email'] = $existing_customers[0]['email'];
            }
    ...
    /**
     * Valida o CPF fornecido para garantir que o paciente já está cadastrado.
     * Retorna um JSON indicando se é válido ou não.
     */
    public function validate_cpf(): void
    {
        try {
            method('post');
            ...
            check('cpf', 'string|null');
            $cpf = request('cpf');
            
            if (empty($cpf)) {
                json_response(['valid' => false, 'message' => 'O CPF é obrigatório.']);
                return;
            }
            ...
            if (empty($existing_customers)) {
                json_response(['valid' => false, 'message' => 'CPF não encontrado.']);
                return;
            }
