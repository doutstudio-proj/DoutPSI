...
App.Pages.Booking = (function () {
    ...
    const STORAGE_KEY = 'DoutPsi.CustomerInfo';
    ...
    function addEventListeners() {
        ...
        $('.button-next').on('click', (event) => {
            ...
            if ($target.attr('data-step_index') === '3') {
                ...
                const cpfValue = $('#custom-field-1').val();
                if (cpfValue) {
                    const originalText = $target.html();
                    $target.prop('disabled', true).text(lang('loading') || 'Aguarde...');

                    $.ajax({
                        url: App.Utils.Url.siteUrl('booking/validate_cpf'),
                        method: 'POST',
                        data: {
                        ...
                        if (response.valid) {
                            ...
                            App.Utils.Message.show('DoutPsi', response.message || 'CPF não encontrado.');
                        }
                    }).fail((jqXHR) => {
                        $target.prop('disabled', false).html(originalText);
                        let msg = 'Erro de comunicação ao validar o CPF.';
                        if (jqXHR && jqXHR.responseJSON && jqXHR.responseJSON.message) {
                            msg = jqXHR.responseJSON.message;
                        }
                        App.Utils.Message.show('DoutPsi', msg);
                    });
                    
                    return; // Prevent default flow, continue inside ajax callback
                } else {
                    App.Pages.Booking.updateConfirmFrame();
