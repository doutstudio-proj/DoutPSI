...
LDAP lets you import users from a central directory (like your company's employee directory) into DoutPsi. It also enables single sign-on (SSO), so users can log in with their existing company credentials.
...
### 3. Link the LDAP entry to an DoutPsi user
...
You can map LDAP fields to DoutPsi fields so that user information is automatically filled in when importing. This is a JSON object, for example:
