...
DoutPsi needs three things on your server: **Apache** (the web server), **PHP** (the programming language), and **MySQL** (the database). You also need the PHP **curl** extension and the Apache **mod_rewrite** module enabled.
...
- `BASE_URL` is set to your exact installation URL (e.g. `http://your-domain.com/DoutPsi`)
...
If DoutPsi is on a subdomain like `http://book.mysite.com`, make sure `BASE_URL` in `config.php` uses the **subdomain URL** — not the folder path.
...
3. Set up DoutPsi in a folder (e.g. `/var/www/html/DoutPsi`)
4. Add this to `/etc/caddy/Caddyfile`:

```Caddyfile
DoutPsi.example.com {
        root * /var/www/html/DoutPsi
        encode gzip zstd
